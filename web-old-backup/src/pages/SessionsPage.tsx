...
const ALL_SOURCES = ["cli", "web", "telegram", "discord", "slack", "whatsapp", "cron"] as const;
...
function getDateGroup(ts: number): string {
  ...
  if (sessionDate >= startOfWeek) return "This Week";
...
function SessionRow({
  ...
  return (
    <div
      className={`border overflow-hidden transition-colors rounded-lg ${
        session.is_active
      ...
      >
        ...
        <div className="flex items-center gap-1 shrink-0">
          ...
          <Button
            ...
            aria-label="Continue session in chat"
...
export default function SessionsPage() {
  ...
  const loadSessions = useCallback((p: number) => {
    ...
    api
      ...
      .then((resp) => {
        const filtered = resp.sessions.filter((s: SessionInfo) => s.message_count > 0);
  ...
  const sourceFiltered = sourceFilter
    ? filtered.filter((s) => s.source === sourceFilter || (sourceFilter === "local" && !s.source))
    : filtered;

  const grouped = useMemo(() => {
    const groups = new Map<string, SessionInfo[]>();
    for (const s of sourceFiltered) {
      const group = getDateGroup(s.last_active);
  ...
  return (
    <div className="flex flex-col gap-4">
        ...
        <Card>
          ...
          <CardContent className="grid gap-3">
            {recentSessions.map((s) => (
              <div
                key={s.id}
                className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 border border-border p-3 w-full rounded-lg hover:bg-secondary/20 transition-colors"
              >
                <div
                  className="flex flex-col gap-1 min-w-0 w-full cursor-pointer"
                  onClick={() => navigate(`/chat?resume=${encodeURIComponent(s.id)}`)}
                >
      ...
      {filtered.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-16 text-muted-foreground glass-panel rounded-xl p-8">
        ...
        <>
          <div className="flex items-center gap-1.5 flex-wrap">
            <button
              ...
              className={`text-[11px] px-2 py-1 rounded-md transition-colors ${!sourceFilter ? "bg-[var(--bg-active)] text-[var(--text-primary)] font-medium" : "text-muted-foreground hover:text-[var(--text-secondary)]"}`}
            >
              All
            </button>
            {ALL_SOURCES.filter(src => sourceFiltered.some(ses => ses.source === src)).map((src) => (
              <button
                key={src}
                onClick={() => setSourceFilter(sourceFilter === src ? null : src)}
                className={`text-[11px] px-2 py-1 rounded-md transition-colors ${sourceFilter === src ? "bg-[var(--bg-active)] text-[var(--text-primary)] font-medium" : "text-muted-foreground hover:text-[var(--text-secondary)]"}`}
              >
                {src}
              </button>
            ))}
          ...
          <div className="flex flex-col gap-1.5">
            {Array.from(grouped.entries()).map(([group, sessions]) => (
              <div key={group}>
                <div className="text-[11px] font-semibold text-muted-foreground px-1 py-2 uppercase tracking-wider">
                  {group}
                </div>
                {sessions.map((s) => (
                  <SessionRow
