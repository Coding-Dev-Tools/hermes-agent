...
import { FileText, RefreshCw, Copy, Check, WrapText } from "lucide-react";
...
export default function LogsPage() {
  ...
  const handleCopyLine = useCallback(async (text: string, index: number) => {
    try {
    ...
    <div className="flex flex-col gap-4">
      ...
      <Card>
        <CardHeader className="py-3 px-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              <Switch checked={wrap} onCheckedChange={setWrap} id="logs-wrap" />
              <Label htmlFor="logs-wrap" className="text-[10px] text-muted-foreground cursor-pointer">
                Wrap
              </Label>
            </div>
          </div>
        ...
        <CardContent className="p-0">
          ...
          <div
            ref={scrollRef}
            className={`p-4 font-mono-ui text-xs leading-5 overflow-auto min-h-[400px] max-h-[calc(100vh-220px)] ${wrap ? "whitespace-pre-wrap break-words" : "whitespace-nowrap"}`}
          >
            {loading && lines.length === 0 && (
              <div className="space-y-2 py-2">
                {Array.from({ length: 12 }).map((_, i) => (
                  <SkeletonLine key={i} width={`w-${[3/4, 2/3, 5/6, 1/2, 4/5, 3/5, 5/6, 2/3, 3/4, 1/2, 4/5, 3/5][i]}`} height="h-3" />
                ))}
              </div>
            )}
            {!loading && lines.length === 0 && (
            ...
            {lines.map((line, i) => {
              ...
              const badgeTone = cls === "error" ? "destructive" : cls === "warning" ? "warning" : "outline";
              return (
                <div
                  key={i}
                  className={`group flex items-start gap-2 ${LINE_COLORS[cls]} hover:bg-secondary/20 px-1 -mx-1`}
                >
                  <Badge tone={badgeTone} className="text-[9px] leading-none mt-[3px] shrink-0 uppercase font-semibold tracking-wider">
                    {cls}
                  </Badge>
                  <span className="flex-1 min-w-0">{line}</span>
                  <button
                    onClick={() => handleCopyLine(line, i)}
                    className="shrink-0 opacity-0 group-hover:opacity-100 transition-opacity p-0.5 rounded hover:bg-[var(--bg-hover)]"
