...
function TokenBarChart({ daily }: { daily: AnalyticsDailyEntry[] }) {
  ...
  const maxTokens = Math.max(
    ...daily.map((d) => d.input_tokens + d.output_tokens + d.cache_read_tokens + d.reasoning_tokens),
    1,
  );
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <div className="flex items-center gap-4 text-xs text-muted-foreground flex-wrap">
          <div className="flex items-center gap-1.5">
            <div className="h-2.5 w-2.5 bg-[#a78bfa]" />
            Cache
          </div>
          <div className="flex items-center gap-1.5">
            <div className="h-2.5 w-2.5 bg-[#f472b6]" />
            Reasoning
          </div>
          <div className="flex items-center gap-1.5">
      ...
      <CardContent>
        ...
        >
          {daily.map((d) => {
            const total = d.input_tokens + d.output_tokens + d.cache_read_tokens + d.reasoning_tokens;
            const cacheH = Math.round((d.cache_read_tokens / maxTokens) * CHART_HEIGHT_PX);
            const reasonH = Math.round((d.reasoning_tokens / maxTokens) * CHART_HEIGHT_PX);
              ...
              >
                <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 hidden group-hover:block z-10 pointer-events-none">
                  <div className="bg-card border border-border px-2.5 py-1.5 text-[10px] text-foreground shadow-lg whitespace-nowrap">
                    ...
                    <div>Cache: {formatTokens(d.cache_read_tokens)}</div>
                    <div>Reasoning: {formatTokens(d.reasoning_tokens)}</div>
                    ...
                    <div>Cost: {formatCost(d.estimated_cost)} (est) / {formatCost(d.actual_cost)}</div>
                  </div>
                </div>

                <div
                  className="w-full bg-[#a78bfa]/70 chart-bar rounded-t-[2px]"
                  style={{
                    height: Math.max(cacheH, d.cache_read_tokens > 0 ? 1 : 0),
                    background: d.cache_read_tokens > 0
                      ? 'linear-gradient(180deg, rgba(167, 139, 250, 0.9) 0%, rgba(167, 139, 250, 0.6) 100%)'
                      : undefined,
                    animationDelay: `${daily.indexOf(d) * 20}ms`,
                  }}
                />
                <div
                  className="w-full bg-[#f472b6]/70 chart-bar rounded-t-[2px]"
                  style={{
                    height: Math.max(reasonH, d.reasoning_tokens > 0 ? 1 : 0),
                    background: d.reasoning_tokens > 0
                      ? 'linear-gradient(180deg, rgba(244, 114, 182, 0.9) 0%, rgba(244, 114, 182, 0.6) 100%)'
                      : undefined,
                    animationDelay: `${daily.indexOf(d) * 20 + 50}ms`,
                  }}
                />
                <div
                  className="w-full bg-[#ffe6cb]/70 chart-bar rounded-t-[2px]"
                  style={{
                    height: Math.max(inputH, total > 0 ? 1 : 0),
                    background: total > 0
                      ? 'linear-gradient(180deg, rgba(255, 230, 203, 0.9) 0%, rgba(255, 230, 203, 0.6) 100%)'
                      : undefined,
                    animationDelay: `${daily.indexOf(d) * 20 + 100}ms`,
                  }}
                />
                <div
                  className="w-full bg-emerald-500/70 chart-bar rounded-t-[2px]"
                  style={{
                    height: Math.max(outputH, d.output_tokens > 0 ? 1 : 0),
                    background: d.output_tokens > 0
                      ? 'linear-gradient(180deg, rgba(16, 185, 129, 0.9) 0%, rgba(16, 185, 129, 0.6) 100%)'
                      : undefined,
                    animationDelay: `${daily.indexOf(d) * 20 + 150}ms`,
                  }}
                />
              </div>
            );
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border text-muted-foreground text-xs">
                ...
                <SortHeader label="Reason" col="reasoning_tokens" sortKey={sortKey} sortDir={sortDir} toggle={toggle} className="text-right py-2 px-4 font-medium" />
...
export default function AnalyticsPage() {
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {loading && !data && <PageSkeleton lines={4} />}
      ...
      {data && (
        <>
              ...
              <CardContent className="py-6">
                <Stats
                  items={[
                    ...
                    {
                      label: "Actual Cost",
                      value: formatCost(data.totals.total_actual_cost),
                    },
                  ]}
                />
              </CardContent>
          ...
          {data.skills.summary.total_skill_loads > 0 && (
            <Card>
              <CardContent className="py-3">
                <div className="flex items-center gap-6 text-xs text-muted-foreground">
                  <span>Total Loads: <strong className="text-foreground">{data.skills.summary.total_skill_loads}</strong></span>
                  <span>Edits: <strong className="text-foreground">{data.skills.summary.total_skill_edits}</strong></span>
                  <span>Actions: <strong className="text-foreground">{data.skills.summary.total_skill_actions}</strong></span>
                  <span>Distinct Skills: <strong className="text-foreground">{data.skills.summary.distinct_skills_used}</strong></span>
                </div>
              </CardContent>
            </Card>
          )}
