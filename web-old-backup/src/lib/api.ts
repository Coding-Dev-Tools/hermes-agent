...
export const api = {
  ...
  // REST chat (fallback when PTY is unavailable)
  chat: async (message: string, sessionId?: string): Promise<{ response: string; session_id: string | null }> => {
    ...
    return data as { response: string; session_id: string | null };
  },

  // File management
  listFiles: (path: string) =>
    fetchJSON<{ path: string; items: FileItem[] }>(`/api/files/list?path=${encodeURIComponent(path)}`),
  readFile: (path: string) =>
    fetchJSON<{ path: string; content: string; size: number }>(`/api/files/content?path=${encodeURIComponent(path)}`),
  writeFile: (path: string, content: string) =>
    fetchJSON<{ ok: boolean; path: string }>("/api/files/content", {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ path, content }),
    }),
  fileTree: (path: string) =>
    fetchJSON<TreeNode>(`/api/files/tree?path=${encodeURIComponent(path)}`),
  createFile: (path: string, isDirectory = false) =>
    fetchJSON<{ ok: boolean; path: string }>("/api/files/create", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ path, is_directory: isDirectory }),
    }),
  deleteFile: (path: string) =>
    fetchJSON<{ ok: boolean }>(`/api/files/delete?path=${encodeURIComponent(path)}`, { method: "DELETE" }),
  renameFile: (oldPath: string, newPath: string) =>
    fetchJSON<{ ok: boolean; path: string }>("/api/files/rename", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
